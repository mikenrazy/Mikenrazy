...
<head>
    ...
    if(isset($_POST['s2']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
       $cid=$_POST['cid'];
       $_SESSION['cid']=$cid;
       $query="select p_id from complaint where c_id='$cid'";
        $rs = $conn->query($query);
        $rows =$rs->fetch_assoc();
        $p_id=$_SESSION['pol'];
     if($rows['p_id']==$p_id){
    ...
    $p_id=$_SESSION['pol'];
     $result="SELECT c_id,type_crime,d_o_c,location FROM complaint where p_id='$p_id' and pol_status='In Process' order by c_id desc";
