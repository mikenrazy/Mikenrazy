...
if(isset($_POST['s']))
{
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        $name=$_POST['email'];
        $pass=$_POST['password'];
       // $result=mysqli_query("SELECT u_id,u_pass FROM user where u_id='$name' and u_pass='$pass' ");
          $query = "SELECT * FROM user  WHERE u_id = '$name' AND U_pass = '$pass'";
					$rs = $conn->query($query);
					$num = $rs->num_rows;
					$rows = $rs->fetch_assoc();
					if($num > 0){
...
<body style="background-size: cover;
  ...
  <div class="container">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="home.php" style="margin-top: 5%;"><b>Crime-Management Portal</b></a>
    </div>
