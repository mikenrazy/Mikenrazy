...
<head>
    ...
    if(isset($_POST['s2']))
    {
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $query1="SELECT a_no FROM complaint where c_id='$cid'";
            $rs = $conn->query($query1);
            $num = $rs->num_rows;
            $rows1 =$rs->fetch_assoc();
           if($rows1['a_no']==$rows['a_no'])
           {
    ...
    $query2="select c_id,type_crime,d_o_c,location from complaint where a_no='$a_no' order by c_id desc";
