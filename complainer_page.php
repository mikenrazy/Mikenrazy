...
session_start();
    ...
    $u_id=$_SESSION['u_id'];
        ...
        $query="SELECT a_no FROM user where u_id='$u_id' ";
        $rs = $conn->query($query);
        $num = $rs->num_rows;
        $rows =$rs->fetch_assoc();
        $a_no=$rows['a_no'];
    
        $query1="SELECT u_name FROM user where u_id='$u_id' ";
	...
	<div class="center-container">
		 <div class="bg-agile">
			...
			<div class="login-form"><p><h2 style="color:white">Welcome <?php echo "$u_name" ?></h2></p><br>
				...
				<form action="#" method="post" style="color: gray">National ID
					<input type="text"  name="National ID_number" placeholder="National ID Number" required="" disabled value=<?php echo "$a_no"; ?>>
				...
				<div class="top-w3-agile" style="color: gray">Type of Crime
					<select class="form-control" name="type_crime">
                        ...
                        <option>Others-(Fill in the description)</option>
				    </select>
				</div>
